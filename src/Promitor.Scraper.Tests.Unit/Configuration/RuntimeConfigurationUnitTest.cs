...
namespace Promitor.Scraper.Tests.Unit.Configuration
{
    ...
    public class RuntimeConfigurationUnitTes
    {
        ...
        public async Task RuntimeConfiguration_IsFullyConfigured_UsesCorrectValues()
        {
            ...
            Assert.NotNull(runtimeConfiguration.FeatureFlags);
            ...
            Assert.Equal(bogusRuntimeConfiguration.FeatureFlags.DisableMetricTimestamps, runtimeConfiguration.FeatureFlags.DisableMetricTimestamps);
        }
        ...
        {
            // Arrange
            var configuration = await RuntimeConfigurationGenerator.WithServerConfiguration(httpPort: null)
                .GenerateAsync();
        ...
        [InlineData(true)]
        [InlineData(false)]
        public async Task RuntimeConfiguration_HasConfiguredFeatureFlagForDisableMetricTimestamps_UsesConfigured(bool disableMetricTimestamps)
        {
            // Arrange
            var configuration = await RuntimeConfigurationGenerator.WithServerConfiguration()
                .WithFeatureFlags(disableMetricTimestamps: disableMetricTimestamps)
                .GenerateAsync();
            ...
            Assert.NotNull(runtimeConfiguration);
            Assert.NotNull(runtimeConfiguration.FeatureFlags);
            Assert.Equal(disableMetricTimestamps, runtimeConfiguration.FeatureFlags.DisableMetricTimestamps);
        }

        [Fact]
        public async Task RuntimeConfiguration_HasNoFeatureFlagForDisableMetricTimestampsConfigured_UsesDefault()
        {
            // Arrange
            var configuration = await RuntimeConfigurationGenerator.WithServerConfiguration()
                .WithFeatureFlags(disableMetricTimestamps: null)
                .GenerateAsync();
            ...
            Assert.NotNull(runtimeConfiguration);
            Assert.NotNull(runtimeConfiguration.FeatureFlags);
            Assert.Equal(Defaults.FeatureFlags.DisableMetricTimestamps, runtimeConfiguration.FeatureFlags.DisableMetricTimestamps);
        }
        ...
        public async Task RuntimeConfiguration_HasConfiguredDefaultTelemetryVerbosityConfigured_UsesConfigured()
        {
            ...
            var configuration = await RuntimeConfigurationGenerator.WithServerConfiguration()
                .WithGeneralTelemetry(defaultVerbosity: defaultVerbosity)
                .GenerateAsync();
        ...
        public async Task RuntimeConfiguration_HasNoDefaultTelemetryVerbosityConfigured_UsesDefault()
        {
            // Arrange
            var configuration = await RuntimeConfigurationGenerator.WithServerConfiguration()
                .WithGeneralTelemetry(defaultVerbosity: null)
                .GenerateAsync();
        ...
        public async Task RuntimeConfiguration_HasConfiguredContainerLogVerbosity_UsesConfigured()
        {
            ...
            var configuration = await RuntimeConfigurationGenerator.WithServerConfiguration()
                .WithContainerTelemetry(verbosity: verbosity)
                .GenerateAsync();
        ...
        public async Task RuntimeConfiguration_HasNoDefaultContainerLogsConfigured_UsesDefault()
        {
            // Arrange
            var configuration = await RuntimeConfigurationGenerator.WithServerConfiguration()
                .WithContainerTelemetry(verbosity: null, isEnabled: null)
                .GenerateAsync();
        ...
        public async Task RuntimeConfiguration_HasConfiguredApplicationInsightsInstrumentationKey_UsesConfigured()
        {
            ...
            var configuration = await RuntimeConfigurationGenerator.WithServerConfiguration()
                .WithApplicationInsightsTelemetry(instrumentationKey: instrumentationKey)
                .GenerateAsync();
