...
namespace Promitor.Tests.Unit.Validation.Scraper.Metrics.ResourceTypes
{
    ...
    public class SqlManagedInstanceMetricsDeclarationValidationStepTests : MetricsDeclarationValidationStepsTests
    {
        ...
        [InlineData(0)]
        [InlineData(10001)]
        public void SqlManagedInstanceMetricsDeclaration_DeclarationWithInvalidMetricLimit_Fails(int metricLimit)
        {
            // Arrange
            var rawDeclaration = MetricsDeclarationBuilder.WithMetadata()
                .WithSqlManagedInstanceMetric(azureMetricLimit: metricLimit)
                .Build(Mapper);
            var metricsDeclarationProvider = new MetricsDeclarationProviderStub(rawDeclaration, Mapper);
