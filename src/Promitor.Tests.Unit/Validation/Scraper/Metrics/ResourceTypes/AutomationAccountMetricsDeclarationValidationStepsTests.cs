...
namespace Promitor.Tests.Unit.Validation.Scraper.Metrics.ResourceTypes
{
    ...
    public class AutomationAccountMetricsDeclarationValidationStepsTests : MetricsDeclarationValidationStepsTests
    {
        ...
        [InlineData(0)]
        [InlineData(10001)]
        public void AutomationAccountMetricsDeclaration_DeclarationWithInvalidMetricLimit_Fails(int metricLimit)
        {
            // Arrange
            var rawDeclaration = MetricsDeclarationBuilder.WithMetadata()
                .WithAutomationAccountMetric(azureMetricLimit: metricLimit)
                .Build(Mapper);
            var metricsDeclarationProvider = new MetricsDeclarationProviderStub(rawDeclaration, Mapper);
