...
namespace Promitor.Tests.Unit.Validation.Scraper.Metrics.ResourceTypes
{
    ...
    public class KubernetesServiceMetricsDeclarationValidationStepsTests : MetricsDeclarationValidationStepsTests
    {
        ...
        [InlineData(0)]
        [InlineData(10001)]
        public void KubernetesServiceMetricsDeclaration_DeclarationWithInvalidMetricLimit_Fails(int metricLimit)
        {
            // Arrange
            var rawDeclaration = MetricsDeclarationBuilder.WithMetadata()
                .WithKubernetesServiceMetric(azureMetricLimit: metricLimit)
                .Build(Mapper);
            var metricsDeclarationProvider = new MetricsDeclarationProviderStub(rawDeclaration, Mapper);
