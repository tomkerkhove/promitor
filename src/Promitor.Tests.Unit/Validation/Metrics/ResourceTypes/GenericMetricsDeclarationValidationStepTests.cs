...
namespace Promitor.Tests.Unit.Validation.Metrics.ResourceTypes
{
    ...
    public class GenericMetricsDeclarationValidationStepTests : MetricsDeclarationValidationStepsTests
    {
        ...
        [Fact]
        public void GenericMetricsDeclaration_DeclarationWithoutResourceButWithResourceCollectionInfo_Succeeds()
        {
            // Arrange
            var rawMetricsDeclaration = MetricsDeclarationBuilder.WithMetadata()
                .WithGenericMetric(omitResource: true, resourceCollectionName: "sample-collection")
                .Build(Mapper);
            var metricsDeclarationProvider = new MetricsDeclarationProviderStub(rawMetricsDeclaration, Mapper);
